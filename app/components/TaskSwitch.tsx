...
export const TaskSwitch = ({ task }: { task: TaskDTO}) => {
    ...
    return (
      <>
        ...
        <Switch
          ...
          onChange={(checked) => {
            if (checked) {
              ...
              submit({ _action: "check" , taskId: id }, {method: "post", navigate: false},);
            } else {
              setIsModalOpen(true);
            }
          }}
        ...
        >
          <span className={`size-4 translate-x-1 rounded-full bg-white transition ${isChecked ?  'translate-x-6': 'translate-x-1'} `} />
        </Switch>
        <Dialog
          open={isModalOpen}
            ...
            <DialogPanel className="max-w-sm space-y-4 border bg-white p-12">
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => {
                    setIsChecked(false);
                    submit({ _action: "uncheck", completionId: task.completionId }, {method: "post", navigate: false});
